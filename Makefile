...
PERLSRC			= find perl scripts -type f \( -name '*.pm' -o \
			  -name '*.t' -o \
			  -exec sh -c 'head -1 "$$1" | grep -q "^\#!.*perl"' \
			  _ {} \; \) -print

DEPS			= perl/sync/scripts/deps
...
.PHONY: all check deps deps-develop deps-test lint prettier prettier-fix test tidy tidy-fix

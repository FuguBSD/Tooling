...
| D-07 | The org pack owns the make dispatcher, and `mk/local.mk` is the consumer hook for build variables and repo-only targets.             | The drift gate then enforces the target contract, and identity stays out of the synced files.       |
| D-08 | A `<verb>` target reads, and a `<verb>-fix` target writes. The names `deps`, `deps-test`, and `deps-develop` must not change.        | One verb set serves every repository, and the shared setup-perl action computes the deps names.     |
| D-09 | A fragment uses the portable make subset. Each dialect gets its own dispatcher: `GNUmakefile` for GNU make, `Makefile` for BSD make. | The fragments serve both dialects, and a new dialect costs one dispatcher file.                     |
